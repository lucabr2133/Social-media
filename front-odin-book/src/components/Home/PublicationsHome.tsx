...
function PublicationHome({ styles, actions, data }: props) {
  ...
  return (
    <div className={`${styles['publication-container']}  flex  md:w-100  m-2   w-full p-0 justify-center  rounded-2xl shadow-2xl`} key={publication.id}>
      <div className='w-full '>

        <div style={{ display: 'flex', alignItems: 'center' }} className='m-1 gap-2'>
          <Link className='ring ring-neutral-700 size-15 rounded-full hover:rounded-4xl!' to={`/profile/${publication.author?.username} `}>
        ...
        <img className='h-[65vh] w-full md:w-full ' src={publication.image_url} alt="" />
        <div className={styles['publication-content']}>
          <div className='flex flex-col gap-5'>
            ...
            <div className='text-neutral-500 font-light capitalize  text-wrap w-full  select-text!' style={{ padding: '20px 10px', fontSize: '12px' }}>{publication.content}</div>
          </div>
        </div>
      </div>
